...
const fetchMeFresh = async (): Promise<User> => {
  const res = await apiService.members.getMe(); // ✅ me()가 아니라 getMe()
...
export class SocialLoginService {
  ...
  /** 토큰 갱신 (백엔드 포맷: { code, result: { accessToken } } 가정) */
  static async refreshAccessToken(): Promise<boolean> {
    ...
    try {
      ...
      const payload = unwrapApi<{ accessToken: string }>(res); // ✅ data 아님, result 언래핑
