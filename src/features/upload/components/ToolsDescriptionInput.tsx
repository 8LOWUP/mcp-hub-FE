// src/features/upload/components/ToolsDescriptionInput.tsx
"use client";
import { useState, useEffect, useRef } from "react";

interface McpTool { name: string; content: string; }
...
const sanitize = (arr?: McpTool[]): McpTool[] => {
    const base = Array.isArray(arr) && arr.length > 0 ? arr : [{ name: "", content: "" }];
    return base.map((t) => ({
...
export default function ToolsDescriptionInput({ initialTools, onChange }: ToolsDescriptionInputProps) {
    // 최초 한 번 초기화
    const [tools, setTools] = useState<McpTool[]>(() => sanitize(initialTools));

    // 🔒 동일한 값이면 setState 생략 (무한 루프 차단)
    const lastInitJson = useRef<string>(JSON.stringify(sanitize(initialTools)));
    ...
    // 변경사항 부모에 반영
    useEffect(() => { onChange?.(tools); }, [tools, onChange]);
    ...
    const addTool = () => {
        setTools((prev) => (prev.length >= 6 ? prev : [...prev, { name: "", content: "" }]));
    };
    ...
    return (
        <div className="mb-6">
            <label className="block mb-2 text-lg font-semibold text-white">Tools Description</label>
            ...
            {tools.map((tool, index) => (
                <div key={index} className="flex items-center gap-2 mb-2">
                    ...
                    {tools.length > 1 && (
                        <button type="button" onClick={() => removeTool(index)} className="text-red-500 hover:text-red-700 font-bold text-lg">
                            ×
                        </button>
                    )}

                    {index === tools.length - 1 && tools.length < 6 && (
                        <button type="button" onClick={addTool} className="px-3 py-3 bg-accent rounded text-black text-sm font-medium hover:opacity-90 transition whitespace-nowrap">
                            + 추가
                        </button>
                    )}
                </div>
